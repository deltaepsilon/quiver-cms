...
angular
  ...
  .run(function(
     ...
     * Auth
     */
    qvAuth.auth.$onAuthStateChanged(function(authData) {
  ...
  .config(function(
     ...
     */
    window.envVars.firebase.endpoint = window.envVars.firebase.endpoint.split('/').pop();
    quiverUtilitiesProvider.setEnv(window.envVars);

    /*
     * Configure qvAuth
     */
    AngularFireAuthenticationProvider.setEndpoint(
      window.envVars.firebase.endpoint.split('/').pop()
    );
    ...
    var states = {
          ...
          user: function($q, $state, qvAuth, AdminService) {
            ...
            return qvAuth
              ...
              .then(function(currentUser) {
                ...
                return qvAuth.getHeaders(currentUser).then(function(headers) {
      ...
      authenticatedMasterNavCheckout: {
        // *****************************  Checkout *************************
        url: '/checkout',
      ...
      authenticatedMasterNavTransaction: {
        // **************************  Transaction **********************
      ...
      authenticatedMasterNavMessages: {
        // *****************************  Messages *************************
        abstract: true,
      ...
      authenticatedMasterNavMessagesList: {
        // *****************************  Messages *************************
        url: '/messages',
        templateUrl: 'views/messages-list.html',
      },
      authenticatedMasterSubscription: {
        // **********************************  Table of Contents ****************
        abstract: true,
      ...
      authenticatedMasterSubscriptionPage: {
        // *********************  User Subscription ****************
        url: '/page/:pageNumber',
        ...
        resolve: {
          isActive: function(UserService, user, $stateParams, $q, NotificationService, $state) {
            ...
            UserService.getSubscription(user.public.id, $stateParams.subscriptionKey)
              .$loaded()
          ...
          word: function(AdminService, UserService, user, $stateParams, $localStorage, $rootScope) {
            ...
            return UserService.getPages(user.public.id, $stateParams.subscriptionKey).then(function(
              pages
            ) {
              ...
              $localStorage['bookmark-' + $stateParams.subscriptionKey] = parseInt(
            ...
            AdminService.getAssignment($stateParams.assignmentKey)
              ...
              .then(function(isExpired) {
                if (isExpired && assignment.startsSubscription) {
                  ...
                  $localStorage['assignment-' + $stateParams.assignmentKey] =
                    $stateParams.assignmentKey;
          ...
          body: {
            ...
            resolve: {
              isAdmin: function(user, $state) {
                if (
                  $state.toState &&
                  $state.current.name.match(/moderator/) &&
                  $state.toState.name !== 'authenticated.master.admin.dashboard'
                ) {
      ...
      authenticatedMasterAdminAssignments: {
        // ************************  Assignments **********************
        abstract: true,
      ...
      authenticatedMasterAdminProducts: {
        // ***************************  Products *************************
        url: '/products',
      ...
      authenticatedMasterAdminSettings: {
        // ***************************  Settings *************************
        url: '/settings',
        ...
        resolve: {
          landingPages: function(AdminService) {
            return AdminService.getLandingPages()
              .$limit(100000)
              .$get();
          },
        },
      ...
      authenticatedMasterAdminSurveys: {
        // ****************************  Surveys **************************
        abstract: true,
      ...
      authenticatedMasterAdminSocial: {
        // *****************************  Social ***************************
        url: '/social-media',
      ...
      authenticatedMasterAdminHashtags: {
        // ***************************  Hashtags *************************
        url: '/hashtags',
      ...
      authenticatedMasterAdminTransactions: {
        // ***********************  Transactions *********************
        abstract: true,
      ...
      authenticatedMasterAdminSubscriptions: {
        // ***********************  Subscriptions *******************
        abstract: true,
      ...
      authenticatedMasterAdminSubscription: {
        // ***********************  Subscription *******************
        ...
        resolve: {
          ...
          userSubscription: function(UserService, subscription, $q) {
            return subscription.$loaded().then(function(subscription) {
              return UserService.getSubscription(
                subscription.user.public.id,
                subscription.keys.user
              );
            });
          },
        },
      },
      authenticatedMasterAdminShipments: {
        // **************************  Shipments ************************
        abstract: true,
      ...
      authenticatedMasterAdminDiscounts: {
        // **************************  Discounts ************************
        abstract: true,
      ...
      authenticatedMasterAdminResources: {
        // **************************  Resources ************************
        abstract: true,
      ...
      authenticatedMasterAdminMessages: {
        // ***************************  Messages *************************
        abstract: true,
      ...
      authenticatedMasterAdminUploads: {
        // ****************************  Uploads **************************
        abstract: true,
      ...
      authenticatedMasterAdminFeedback: {
        // ***************************  Assignment ***********************
        ...
        resolve: {
          ...
          assignmentUploads: function(UserService, $stateParams) {
            return UserService.getAssignmentUploads(
              $stateParams.userId,
              $stateParams.assignmentKey
            );
          },
          assignmentMessages: function(UserService, $stateParams) {
            return UserService.getAssignmentMessages(
              $stateParams.userId,
              $stateParams.assignmentKey
            );
          },
      ...
      authenticatedMasterAdminLandingPages: {
        // ***********************  Landing Pages ********************
        url: '/landing-pages',
      ...
      authenticatedMasterModeratorDashboard: {
        // ***************************  Moderator ********************
        url: '/dashboard',
      ...
      authenticatedMasterModeratorMessages: {
        // ***************************  Messages *************************
        abstract: true,
            ...
            if (!user.permissions || !user.permissions.assignments) {
              ...
              AdminService.getAllAssignments()
                ...
                .then(function(assignments) {
                  ...
                  deferred.resolve(
                    _.filter(assignments, function(assignment) {
      ...
      authenticatedMasterModeratorUploads: {
        // ****************************  Uploads **************************
        abstract: true,
    ...
    $stateProvider
      ...
      .state(
        'authenticated.master.nav.transaction',
        getState(states.authenticatedMasterNavTransaction)
      )
      .state('authenticated.master.nav.messages', getState(states.authenticatedMasterNavMessages))
      .state(
        'authenticated.master.nav.messages.list',
        getState(states.authenticatedMasterNavMessagesList)
      )
      .state('authenticated.master.subscription', getState(states.authenticatedMasterSubscription))
      .state(
        'authenticated.master.subscription.page',
        getState(states.authenticatedMasterSubscriptionPage)
      )
      .state(
        'authenticated.master.subscription.assignment',
        getState(states.authenticatedMasterSubscriptionAssignment)
      )
      .state(
        'authenticated.master.nav.find-assignment',
        getState(states.authenticatedMasterFindassignment)
      )
      .state(
        'authenticated.master.nav.archivedGallery',
        getState(states.authenticatedMasterArchivedGallery)
      )
      .state('authenticated.master.admin', getState(states.authenticatedMasterAdmin)) // Admin
      .state(
        'authenticated.master.admin.dashboard',
        getState(states.authenticatedMasterAdminDashboard)
      )
      .state('authenticated.master.admin.words', getState(states.authenticatedMasterAdminWords))
      .state(
        'authenticated.master.admin.words.list',
        getState(states.authenticatedMasterAdminWordsList)
      )
      .state('authenticated.master.admin.word', getState(states.authenticatedMasterAdminWord))
      .state(
        'authenticated.master.admin.assignments',
        getState(states.authenticatedMasterAdminAssignments)
      )
      .state(
        'authenticated.master.admin.assignments.list',
        getState(states.authenticatedMasterAdminAssignmentsList)
      )
      .state(
        'authenticated.master.admin.assignment',
        getState(states.authenticatedMasterAdminAssignment)
      )
      .state('authenticated.master.admin.files', getState(states.authenticatedMasterAdminFiles))
      .state(
        'authenticated.master.admin.files.list',
        getState(states.authenticatedMasterAdminFilesList)
      )
      .state('authenticated.master.admin.product', getState(states.authenticatedMasterAdminProduct))
      .state(
        'authenticated.master.admin.products',
        getState(states.authenticatedMasterAdminProducts)
      )
      .state('authenticated.master.admin.users', getState(states.authenticatedMasterAdminUsers))
      .state(
        'authenticated.master.admin.users.list',
        getState(states.authenticatedMasterAdminUsersList)
      )
      .state('authenticated.master.admin.user', getState(states.authenticatedMasterAdminUser))
      .state(
        'authenticated.master.admin.settings',
        getState(states.authenticatedMasterAdminSettings)
      )
      .state(
        'authenticated.master.admin.commerce',
        getState(states.authenticatedMasterAdminCommerce)
      )
      .state('authenticated.master.admin.surveys', getState(states.authenticatedMasterAdminSurveys))
      .state(
        'authenticated.master.admin.surveys.list',
        getState(states.authenticatedMasterAdminSurveysList)
      )
      ...
      .state(
        'authenticated.master.admin.hashtags',
        getState(states.authenticatedMasterAdminHashtags)
      )
      .state(
        'authenticated.master.admin.transactions',
        getState(states.authenticatedMasterAdminTransactions)
      )
      .state(
        'authenticated.master.admin.transactions.list',
        getState(states.authenticatedMasterAdminTransactionsList)
      )
      .state(
        'authenticated.master.admin.transaction',
        getState(states.authenticatedMasterAdminTransaction)
      )
      .state(
        'authenticated.master.admin.subscriptions',
        getState(states.authenticatedMasterAdminSubscriptions)
      )
      .state(
        'authenticated.master.admin.subscriptions.list',
        getState(states.authenticatedMasterAdminSubscriptionsList)
      )
      .state(
        'authenticated.master.admin.subscription',
        getState(states.authenticatedMasterAdminSubscription)
      )
      .state(
        'authenticated.master.admin.shipments',
        getState(states.authenticatedMasterAdminShipments)
      )
      .state(
        'authenticated.master.admin.shipments.list',
        getState(states.authenticatedMasterAdminShipmentsList)
      )
      .state('authenticated.master.admin.reports', getState(states.authenticatedMasterAdminReports))
      .state(
        'authenticated.master.admin.discounts',
        getState(states.authenticatedMasterAdminDiscounts)
      )
      .state(
        'authenticated.master.admin.discounts.list',
        getState(states.authenticatedMasterAdminDiscountsList)
      )
      .state(
        'authenticated.master.admin.resources',
        getState(states.authenticatedMasterAdminResources)
      )
      .state(
        'authenticated.master.admin.resources.list',
        getState(states.authenticatedMasterAdminResourcesList)
      )
      .state(
        'authenticated.master.admin.messages',
        getState(states.authenticatedMasterAdminMessages)
      )
      .state(
        'authenticated.master.admin.messages.list',
        getState(states.authenticatedMasterAdminMessagesList)
      )
      .state('authenticated.master.admin.uploads', getState(states.authenticatedMasterAdminUploads))
      .state(
        'authenticated.master.admin.uploads.list',
        getState(states.authenticatedMasterAdminUploadsList)
      )
      .state(
        'authenticated.master.admin.feedback',
        getState(states.authenticatedMasterAdminFeedback)
      )
      .state('authenticated.master.admin.email', getState(states.authenticatedMasterAdminEmail))
      .state(
        'authenticated.master.admin.email.list',
        getState(states.authenticatedMasterAdminEmailList)
      )
      .state(
        'authenticated.master.admin.landing-pages',
        getState(states.authenticatedMasterAdminLandingPages)
      )
      .state(
        'authenticated.master.admin.landing-page',
        getState(states.authenticatedMasterAdminLandingPage)
      )
      .state('authenticated.master.admin.logs', getState(states.authenticatedMasterAdminLogs))
      .state(
        'authenticated.master.admin.logs.type',
        getState(states.authenticatedMasterAdminLogsType)
      )
      ...
      .state(
        'authenticated.master.moderator.dashboard',
        getState(states.authenticatedMasterModeratorDashboard)
      )
      .state(
        'authenticated.master.moderator.messages',
        getState(states.authenticatedMasterModeratorMessages)
      )
      .state(
        'authenticated.master.moderator.messages.list',
        getState(states.authenticatedMasterModeratorMessagesList)
      )
      .state(
        'authenticated.master.moderator.uploads',
        getState(states.authenticatedMasterModeratorUploads)
      )
      .state(
        'authenticated.master.moderator.uploads.list',
        getState(states.authenticatedMasterModeratorUploadsList)
      )
      .state(
        'authenticated.master.moderator.feedback',
        getState(states.authenticatedMasterAdminFeedback)
      )
      .state('authenticated.master.moderator.words', _.clone(states.authenticatedMasterAdminWords))
      .state(
        'authenticated.master.moderator.words.list',
        _.clone(states.authenticatedMasterAdminWordsList)
      )
      .state('authenticated.master.moderator.word', getState(states.authenticatedMasterAdminWord))
      .state(
        'authenticated.master.moderator.assignments',
        getState(states.authenticatedMasterAdminAssignments)
      )
      .state(
        'authenticated.master.moderator.assignments.list',
        getState(states.authenticatedMasterAdminAssignmentsList)
      )
      .state(
        'authenticated.master.moderator.assignment',
        getState(states.authenticatedMasterAdminAssignment)
      )
      .state('authenticated.master.moderator.files', getState(states.authenticatedMasterAdminFiles))
      .state(
        'authenticated.master.moderator.files.list',
        getState(states.authenticatedMasterAdminFilesList)
      )
      .state(
        'authenticated.master.moderator.transactions',
        getState(states.authenticatedMasterAdminTransactions)
      )
      .state(
        'authenticated.master.moderator.transactions.list',
        getState(states.authenticatedMasterAdminTransactionsList)
      )
      .state(
        'authenticated.master.moderator.transaction',
        getState(states.authenticatedMasterAdminTransaction)
      )
      .state(
        'authenticated.master.moderator.discounts',
        getState(states.authenticatedMasterAdminDiscounts)
      )
      .state(
        'authenticated.master.moderator.discounts.list',
        getState(states.authenticatedMasterAdminDiscountsList)
      )
      .state(
        'authenticated.master.moderator.subscriptions',
        getState(states.authenticatedMasterAdminSubscriptions)
      )
      .state(
        'authenticated.master.moderator.subscriptions.list',
        getState(states.authenticatedMasterAdminSubscriptionsList)
      )
      .state(
        'authenticated.master.moderator.subscription',
        getState(states.authenticatedMasterAdminSubscription)
      )
      .state(
        'authenticated.master.moderator.shipments',
        getState(states.authenticatedMasterAdminShipments)
      )
      .state(
        'authenticated.master.moderator.shipments.list',
        getState(states.authenticatedMasterAdminShipmentsList)
      )
      .state(
        'authenticated.master.moderator.resources',
        getState(states.authenticatedMasterAdminResources)
      )
      .state(
        'authenticated.master.moderator.resources.list',
        getState(states.authenticatedMasterAdminResourcesList)
      )
      .state('authenticated.master.moderator.email', getState(states.authenticatedMasterAdminEmail))
      .state(
        'authenticated.master.moderator.email.list',
        getState(states.authenticatedMasterAdminEmailList)
      )
      .state('authenticated.master.moderator.users', getState(states.authenticatedMasterAdminUsers))
      .state(
        'authenticated.master.moderator.users.list',
        getState(states.authenticatedMasterAdminUsersList)
      );
  });

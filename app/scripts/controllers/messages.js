...
angular.module('quiverCmsApp')
    .controller('MessagesCtrl', function($scope, $q, messageable, sentMessages, receivedMessages, UserService, NotificationService, _) {
        ...
        $scope.addUser = function(userId) {
            var user = _.findWhere($scope.newRecipients, {
                "$id": userId
            });

            if (user && !_.findWhere($scope.uniqueUsers, {
                    id: user.userKey
                })) {
        ...
        $scope.getMessages = function(userId) {
            var sent = _.where($scope.sentMessages, {
                    recipientId: userId
                }),
                received = _.where($scope.receivedMessages, {
                    senderId: userId
                }),
        ...
        $scope.sendMessage = function(userId, recipientId, message) {
            UserService.sendMessage(userId, recipientId, message).then(function() {
                ...
                NotificationService.error('Message failed', err.data || error.statusText || JSON.stringify(error));
            });
        };

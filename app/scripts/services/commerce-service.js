...
/**
 * @ngdoc service
 * @name quiverCmsApp.CommerceService
 * @description
 * # CommerceService
...
angular.module('quiverCmsApp')
    .service('CommerceService', function CommerceService($filter, _, Restangular, Analytics) {
        ...
        var addProducts = function(cart) {
                _.each(cart.items, function(item) {
                    var productId = item.slug,
                        name = item.title,
                        category = item.type,
                        brand = item.brand,
                        variant = item.optionsMatrixSelected ? item.optionsMatrixSelected.slug : 'Default Variant',
            ...
            addCodes = function(cart) {
                _.each(cart.codes, function(code) {
                    var productId = code.code,
                        name = code.type === 'value' ? $filter('currency')(code.value) : code.percentage + '%',
                        category = code.type,
                        brand,
                        variant,
                        price = -1 * code.discount,
                        quantity = 1,
                        coupon = code.code,
                        position;
                    Analytics.addProduct(productId, name, category, brand, variant, price, quantity, coupon, position);
        ...
        return {
            ...
            getClientToken: function() {
                return Restangular.one('user').one('payment').one('token').get();
            },

            createPaymentMethod: function(nonce) {
                return Restangular.one('user').one('payment').one(nonce).post('nonce');
            },

            removePaymentMethod: function(token) {
            ...
            checkout: function(cart) {
                return Restangular.one('user').post('checkout', {
                    cart: cart
                });
            },
            ...
            isShipped: function(product) {
                return !product ? false : product.shipped || !!_.findWhere(product.optionsMatrix, {
                    shipped: true
                }) || false;
            }
        }

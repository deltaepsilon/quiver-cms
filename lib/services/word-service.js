    ...
    renderLandingPage: function(slug, viewsDir) {
        ...
        Q.all([ObjectService.getTheme(), ObjectService.getSettings(), fsDeferred.promise]).spread(function(theme, settings, fsRes) {
            var context = {
                ...
                settings: settings,
                theme: theme,
                title: settings.seo && settings.seo.title ? settings.seo.title : settings.siteTitle,
                description: settings.seo && settings.seo.description ? settings.seo.description : false

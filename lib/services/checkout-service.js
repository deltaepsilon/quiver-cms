...
module.exports = {
	createTransaction: function (user, cart) {
		var transaction = {
				...
				transactionCount: user.private.commerce && user.private.commerce.transactions && user.private.commerce.transactions.length ? user.private.commerce.transactions.length + 1 : 1,
				items: [],
				date: moment().format()
			};
	...
	createDiscounts: function (transaction) {
		...
		_.each(discounts, function (discount) {
			...
			while (i--) { // Users can purchase multiple of each gift certificate, so generate as many discounts as required by discount.quantity
				newDiscount = {
					active: true,
					code: generateCode(transaction.transactionCount + transaction.userId.replace(/.+:/, ''), 6),
				...
				if (err) {
					subscriptionDeferred.reject(err);
			 		...
			 		FirebaseService.authWithSecret(subscriptionLogRef).then(function (ref) {
			 			ref.set(subscription, function (err) {
		...
		transactionRef.set(cleanedTransaction, function (err) {
			if (err) {
				...
				FirebaseService.authWithSecret(transactionLogRef).then(function (ref) {
					ref.set(cleanedTransaction, function (err) {
	...
	updateTransaction: function (transaction) {
		...
		userTransactionRef.set(cleanedTransaction, function (err) {
			if (err) {
				deferred.reject(err);
			} else {
				transactionRef.set(cleanedTransaction, function (err) {

module.exports = function (firebaseRoot) {
	var config = require('config'),
		...
		selfFulfilling = function (transaction) {
	...
	return {
		createTransaction: function (user, cart) {
			...
			return selfFulfilling(transaction);
		},
		...
		sendTransactionEmail: function (key, transaction) {
			...
			request.get(config.get('public.root') + '/transaction/' + key + '/email/text').end(function (err, res) {
        return err ? textDeferred.reject(err) : textDeferred.resolve(res.text);
      });

      request.get(config.get('public.root') + '/email/transaction/' + key + '/email/html').end(function (err, res) {
      	var content = res.body;
      	console.log('html err, content', err, content);

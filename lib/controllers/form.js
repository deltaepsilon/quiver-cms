    ...
    form.parse(req, function (err, fields, files) {
      ...
      var flowFilename = fields.flowFilename,
        ...
        concatNextChunk = function (i) {
          var i = i || 1,
            chunk = _.findWhere(chunks, {
              flowFilename: flowFilename,
              flowChunkNumber: i.toString()
            }),
      ...
      if (flowChunkNumber === flowTotalChunks) {
        ...
        if (flowChunkNumber % 5 === 0) { // Set notification on every fifth chunk
          var notificationsRef = req.userRef.child('notifications').child(slug(flowFilename, {
            charmap: {
              '.': '-',
              '&': 'and'
            }

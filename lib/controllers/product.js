...
module.exports = {
		...
		Q.all([ObjectService.getSettings(), ObjectService.getProducts()]).spread(function (settings, products) {
			...
			var hashtag = req.params.hashtag,
				...
				matching = _.filter(products, function (product) {
					var match = _.findWhere(product.hashtags, {key: hashtag});
					if (!match) {
						return false;
					} else if (!title) {
	...
	product: function (req, res) {
		Q.all([ObjectService.getSettings(), ObjectService.getProducts()]).spread(function (settings, products) {
		    ...
		    useInventoryMatrix = product.optionsMatrix && _.find(product.optionsMatrix, function (option) { return typeof option.inventory !== undefined; }),
		    inStock = false;

	    if (product.optionsMatrix && _.findWhere(product.optionsMatrix, {inStock: true})) {
	    	inStock = true;

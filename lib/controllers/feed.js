...
service = {
  ...
  updateFulfillment: function(req, res) {
    var lineItems = req.body.order['line_items'];
    var rootRef;
    var rootRegex;
    var slugMap = {
      usps: 'usps',
      ...
      .then(function(updates) {
        return Q.all(_.map(updates, function(update) {
            return new Q.Promise(function(resolve, reject) {
              var carrierSlug = slugMap[update.update.carrier.toLowerCase()];
              var trackingNumber = update.update.tracking_id.split(' ').pop();
              var tracking = {
                slug: carrierSlug,

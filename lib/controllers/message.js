    ...
    log: function(req, res) {
        ...
        FirebaseService.authWithSecret(messagesRef).then(function(ref) {
            ...
            if (assignmentKey && !user.isModerator) { // Don't duplicate moderator comments in the moderator logs
                ...
                // Admin uploads don't have assignment keys!
                // LogService.email('assignmentKey missing for user ' + userId);
            }
    ...
    upload: function(req, res) {
        ...
        readDeferred.promise.then(function(data) {
            var file = new Buffer(data, "base64"),
                payload = {
                    Bucket: publicBucket,
                    Key: fileUserPrefix + '/' + userId + '/' + shortid.generate() + ':' + fileName.replace(/#/g, '-').replace(/:/g, "/"), // Filenames with forward slashes in them get copied to the filesystem with ":" instead of the slashes.
                    ACL: 'public-read',
                    Body: file,
        ...
        fileDeferred.promise.then(function () {
            return FirebaseService.authWithSecret(FirebaseService.firebaseRoot);
        }).then(function (ref) {
            rootRef = ref;
        }).then(function() {
            return FileService.updateUserFilesRegister(userId);
        }).then(function(s3Data) {
            var upload = _.findWhere(s3Data.Contents, {
                    ETag: ETag
                }),
            ...
            var adminUploadRef = FirebaseService.chainFromRef(rootRef, FirebaseService.getUploads().toString()).push();
            var moderatorUploadRef = FirebaseService.chainFromRef(rootRef, FirebaseService.getModeratorUploads(record.assignmentKey).toString()).push();
            var rootRegex = new RegExp(rootRef.toString());
            var updates = {};

            record.keys = {
                log: adminUploadRef.key(),
                moderator: moderatorUploadRef.key()
            };
            updates[adminUploadRef.toString().replace(rootRegex, '')] = record;
            updates[moderatorUploadRef.toString().replace(rootRegex, '')] = record;

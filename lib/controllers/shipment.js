...
module.exports = {
  ...
  updateTracking: function (req, res) {
    EasypostService.updateTracking(req.params.shipmentKey, req.params.labelKey, req.body.tracking, req.body.email, req.body.sms)
    .then(updateLabel(req.params.shipmentKey))
    .then(function (shipment) {
      res.json({shipment: shipment});
    }, function (error) {
      console.log('updateTracking error', error);
      res.status(500).send(error);
    });
  }

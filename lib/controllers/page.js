...
module.exports = {
	...
	frontPage: function (req, res) {
		Q.all([ObjectService.getSettings(), ObjectService.getTheme()]).spread(function (settings, theme) {
			WordService.renderPosts(theme && theme.frontPage ? theme.frontPage : 'front-page', 0, req.url, {title: settings && settings.siteTitle ? settings.siteTitle : 'Quiver CMS'}).then(function (html) {
